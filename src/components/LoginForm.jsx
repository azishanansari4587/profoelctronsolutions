...
const LoginForm = ({ className, ...props }) => {
  ...
  const handleVerifyOTP = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/verifyLoginOtp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: formData.email,
  ...
  return (
    <div className={cn("flex justify-center items-center min-h-screen bg-gray-50 px-4", className)} {...props}>
      <Card className="w-full max-w-md shadow-lg border">
        ...
        {step === 3 && (
          <CardContent className="text-center space-y-3">
            <h3 className="text-green-600 font-bold text-xl">🎉 Login Successful!</h3>
